...
namespace JsonTestAdapter
{
    ...
    public class JsonTestContainerDiscoverer : ITestContainerDiscoverer
    {
        ...
        protected string FileExtension { get { return ".json"; } }
        ...
        public JsonTestContainerDiscoverer(
        ...
        private void OnSolutionProjectChanged(object sender, SolutionEventsListenerEventArgs e)
        {
            if (e != null)
            {
                var files = FindJsonFiles(e.Project);
        ...
        private void AddTestContainerIfTestFile(string file)
        {
            ...
            if (isTestFile)
            {
                var container = new JsonTestContainer(this, file, ExecutorUri);
        ...
        private IEnumerable<ITestContainer> GetTestContainers()
        {
            if (initialContainerSearch)
            {
                ...
                UpdateFileWatcher(jsonFiles, true);
        ...
        private IEnumerable<string> FindJsonFiles()
        {
            ...
            return loadedProjects.SelectMany(FindJsonFiles).ToList();
        }

        private IEnumerable<string> FindJsonFiles(IVsProject project)
        {
        ...
        {
            return ".json".Equals(Path.GetExtension(path), StringComparison.OrdinalIgnoreCase);
        }
